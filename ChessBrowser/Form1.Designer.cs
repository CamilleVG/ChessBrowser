        ...
        private void UploadGamesToDatabase(string PGNfilename)
        {
            ...
            using (MySqlConnection conn = new MySqlConnection(connection))
            {
                try
                {
                    ...
                    //new Thread(insert ignore); //batch commands //convert ifs to sql where
                    foreach (ChessGame game in games)
                    {
                        cmd.CommandText = "insert ignore into Players(Name, Elo) values(\"" + game.BlackName + "\", 2002) ON DUPLICATE KEY UPDATE Elo = IF( " + game.BlackElo + " > Elo, " + game.BlackElo + ", Elo);" +
                            "insert ignore into Players(Name, Elo) values(\"" + game.WhiteName + "\", 2002) ON DUPLICATE KEY UPDATE Elo = IF( " + game.WhiteElo + " > Elo, " + game.BlackElo + ", Elo);" +
                            "insert ignore into Events(Name, Date, Site) values (\"" + game.Event + "\", \"" + game.EventDate + "\", \"" + game.Site + "\");" +
                            "insert ignore into Games(Round, Result, Moves, BlackPlayer, WhitePlayer, eID) values(" + game.Round + ", '" + game.Result + "' , \"" + game.Moves + "\", (select pID from Players where Name = \"" + game.BlackName + "\"), (select pID from Players where Name = \"" + game.WhiteName + "\"), (select eID from Events where Name = \"" + game.Event + "\" AND Date = \"" + game.EventDate + "\" AND Site = \"" + game.Site + "\"));";
